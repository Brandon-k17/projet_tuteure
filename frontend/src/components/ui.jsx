...
export function NavIcon({ id }) {
  ...
  switch (id) {
    ...
    case "alert2": return <svg {...p}><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>;
...
// Calendrier avec icône
export function DatePicker({ label, value, onChange, required, min }) {
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
      ...
      <div style={{ position:"relative", display:"flex", alignItems:"center" }}>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
          stroke={C.textLight} strokeWidth="1.8" strokeLinecap="round"
        ...
        <input
          type="date" value={value} onChange={onChange} required={required} min={min}
          style={{ ...S.search, width:"100%", paddingLeft:34, cursor:"pointer" }}
        />
      </div>
    </div>
  );
...
// Montre avec icône
export function TimePicker({ label, value, onChange, required }) {
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
      <label style={{ fontSize:11, fontWeight:700, color:C.textMid,
        textTransform:"uppercase", letterSpacing:"0.8px" }}>{label}</label>
      <div style={{ position:"relative", display:"flex", alignItems:"center" }}>
        ...
        <input
          type="time" value={value} onChange={onChange} required={required}
          style={{ ...S.search, width:"100%", paddingLeft:34, cursor:"pointer" }}
        />
      </div>
    </div>
